...
description: Learn about the parameters used in forecast model training in Vertex AI.
...
Taken together, the rows in the window define a time-series instance that serves as a model input: it is what Vertex AI trains on, evaluates on, and uses for inference. The row used to generate the window is the first row of the horizon and uniquely identifies the window in the time series.
...
When dealing with covariates that are *available* at forecast time, Vertex AI considers covariate values from both the context window and the forecast horizon for training, evaluation, and inference. When dealing with covariates that are *unavailable* at forecast time, Vertex AI considers covariate values from the context window but explicitly excludes covariate values from the forecast horizon.
