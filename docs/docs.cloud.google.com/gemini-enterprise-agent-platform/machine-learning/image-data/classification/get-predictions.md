---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/image-data/classification/get-predictions
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/image-data/classification/get-predictions
title: Get inferences from an image classification model
description: Learn how to get inferences from an image classification model.
data_source: docs.cloud.google.com
---

This page shows you how to get online (real-time) inferences and batch inferences from your image classification models using the Google Cloud console or the Agent Platform API.
...
2.  Click the name of the model you want to deploy to open its **Model description** page.

3.  In the **Version ID** column, click the ID of the version of the model you want to deploy
...
6.  To deploy your model to a new endpoint, click radio\_button\_checked **Create new endpoint** and enter a name for the new endpoint. To deploy your model to an existing endpoint, click radio\_button\_checked **Add to existing endpoint** and select the endpoint **Endpoint name** .
    
    You can add more than one model to an endpoint, and you can add a model to multiple endpoints. [Learn more](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/general/deployment) .
    
    If you deploy to a new endpoint, choose how your endpoint can be accessed:
    
      - Click **Standard** for the endpoint endpoint to be available for inference through a REST API.
...
4.  Under the **Test your model** section, add test items to request an inference.
    ...
    For information about local feature importance, see [Get explanations](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/image-data/classification/get-predictions#explanations-api) .
    
    After the inference is complete, Gemini Enterprise Agent Platform returns the results in the console.

### API

Use the Agent Platform API to request an online inference. Your model must be deployed to an endpoint. For a more detailed example, see the tutorial notebook for [online prediction with an AutoML image classification model](https://github.com/GoogleCloudPlatform/vertex-ai-samples/blob/main/notebooks/official/automl/automl_image_classification_online_prediction.ipynb) .
...
> **Note:** You must select input and output locations that are in the same region as your model. If you use the API to create batch inferences, send requests to a service endpoint (such as `https://us-central1-aiplatform.googleapis.com` ) that is in the same region as your input and output locations.

### Input data requirements

The input for batch requests specifies the items to send to your model for inference. For image classification models, you can use a JSON Lines file to specify a list of images to make inferences about and then store the JSON Lines file in a Cloud Storage bucket. The following sample shows a single line in an input JSON Lines file:
...
The following an example batch inference results from an image classification model.
