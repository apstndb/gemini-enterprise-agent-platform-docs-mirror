---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/optimize/evaluation/evaluate-offline
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/optimize/evaluation/evaluate-offline
title: Run offline evaluations
description: How to run offline agent evaluations Gemini Enterprise Agent Platform.
...
Offline evaluation requires your agent to export specific OpenTelemetry signals to provide the necessary context for assessment. These requirements are identical to those for [Online Monitors](https://docs.cloud.google.com/gemini-enterprise-agent-platform/optimize/evaluation/evaluate-online#telemetry-requirements) :
...
4.  Use the filter icon and time picker to filter data (for example, by **Version** or "Last 2 weeks") and select the specific IDs you want to evaluate.

5.  Click **Continue** .

6.  (Optional) In the **Evaluation name** field, enter a name for the assessment or use the pre-filled default.

7.  In the **Output private data path** field, enter your **Cloud Storage** bucket URI. After the first use, this path is pre-filled for future runs.

8.  By default, all four core metrics are added. You can add or remove metrics as needed.
