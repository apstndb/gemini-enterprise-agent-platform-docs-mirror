...
| `projects/         PROJECT_ID        /logs/cloudaudit.googleapis.com%2Fdata_access`             | Cloud Audit Logs for Identity-Aware Proxy `AuthorizeUser` decisions.  |
...
If you use Model Armor for content sanitization, those logs are emitted separately. For instructions on querying Model Armor logs, see [View raw content security logs in Logs Explorer](https://docs.cloud.google.com/gemini-enterprise-agent-platform/govern/monitor-content-security#view-raw-logs) .

> **Important:** When querying logs, a narrow freshness window (for example, `--freshness=6h` in command line queries) can sometimes return zero rows even if logs were recently generated. If your query returns empty results, try widening the search window (for example, to `--freshness=1d` ) to check if logging is disabled.
