---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/reference/mcp/tools_list/get_prompt
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/reference/mcp/tools_list/get_prompt
...
Retrieves full details of a prompt including metadata, labels, and configuration. Use this to inspect a prompt's settings or retrieve its full resource name before an update. IMPORTANT: This parameter requires the numeric ID or UUID returned by the list tool, NOT the display name. CRITICAL: For {region}, use the region specified in the current context window. If no region is specified, prompt the user to provide one. Do not use 'global'.
