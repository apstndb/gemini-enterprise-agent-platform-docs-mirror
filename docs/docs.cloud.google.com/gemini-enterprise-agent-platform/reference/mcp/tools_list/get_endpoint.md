---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/reference/mcp/tools_list/get_endpoint
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/reference/mcp/tools_list/get_endpoint
...
Retrieves the full metadata for a specific Agent Platform Endpoint, including its operational status, current traffic split across models, and details of all deployed model versions. Use this to inspect the health of a serving endpoint or to verify if a model deployment has completed successfully. Format: 'projects/{project\_id}/locations/{region}'. CRITICAL: For {region}, use the region specified in the current context window. If no region is specified, prompt the user to provide one. Do not use 'global'. IMPORTANT: This parameter requires the numeric ID or UUID returned by the list tool, NOT the display name.

The following sample demonstrate how to use `curl` to invoke the `get_endpoint` MCP tool.
