---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/reference/mcp/tools_list/colab_enterprise_list_notebook_runtime_templates
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/reference/mcp/tools_list/colab_enterprise_list_notebook_runtime_templates
...
Lists all Colab Enterprise runtime templates within a specified Google Cloud location. Use this tool to discover available configurations before provisioning a notebook runtime. Format: 'projects/{project\_id}/locations/{region}'. CRITICAL: For {region}, use the region specified in the current context. If no region is specified, prompt the user for one. Do not use 'global'. IMPORTANT: If the returned list is empty, it means no templates currently exist in this location; this is NOT a system failure. In this case, you should inform the user and ask if they would like to create a new template using the colab\_enterprise\_create\_notebook\_runtime\_template tool.
