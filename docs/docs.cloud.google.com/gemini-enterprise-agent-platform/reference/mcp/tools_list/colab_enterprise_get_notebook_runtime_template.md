---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/reference/mcp/tools_list/colab_enterprise_get_notebook_runtime_template
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/reference/mcp/tools_list/colab_enterprise_get_notebook_runtime_template
...
Retrieves the full metadata for a specific Colab Enterprise runtime template. Use this tool to inspect template configurations. Format: 'projects/{project\_id}/locations/{region}/notebookRuntimeTemplates/{notebook\_runtime\_template\_id}'. CRITICAL: For {region}, use the region specified in the current context. If no region is specified, prompt the user for one. Do not use 'global'.
