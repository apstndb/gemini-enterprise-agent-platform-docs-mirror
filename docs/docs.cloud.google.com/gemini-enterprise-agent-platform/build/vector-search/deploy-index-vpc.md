---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/build/vector-search/deploy-index-vpc
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/build/vector-search/deploy-index-vpc
title: Deploy and manage index endpoints in a VPC network
description: Deploy and manage index endpoints in a VPC network.
...
Before you use an index to serve online vector matching queries, you must deploy the `Index` to an `IndexEndpoint` within your [VPC Network Peering network](https://docs.cloud.google.com/gemini-enterprise-agent-platform/build/vector-search/vpc) . The first step is to create an `IndexEndpoint` . You can deploy more than one index to an `IndexEndpoint` that shares the same VPC network.
...
Before using any of the command data below, make the following replacements:
  ...
  - VPC\_NETWORK\_NAME : The Google Compute Engine network name to which the index endpoint should be peered.
...
5.  Click the more\_vert options menu that is in the same row as the index endpoint and select **Undeploy** .
...
Before you delete an `IndexEndpoint` , you must [undeploy](https://docs.cloud.google.com/gemini-enterprise-agent-platform/build/vector-search/deploy-index-vpc#undeploy-index) all indexes deploy to the endpoint.
...
3.  All of the existing index endpoints are displayed.
4.  Click the more\_vert options menu that is in the same row as the index endpoint you want to delete and select **Delete** .
