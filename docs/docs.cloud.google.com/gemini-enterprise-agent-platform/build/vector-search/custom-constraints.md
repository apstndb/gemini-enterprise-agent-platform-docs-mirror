---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/build/vector-search/custom-constraints
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/build/vector-search/custom-constraints
title: Create custom constraints for Vector Search
description: Learn how to manage Vector Search resources with custom constraints
...
Replace the following:
  ...
  - `  RESOURCE_NAME  ` : the fully qualified name of the Google Cloud resource containing the object and field that you want to restrict. For example, `aiplatform.googleapis.com/Index` . Most resource types support up to 20 custom constraints. If you attempt to create more custom constraints, the operation fails.
  ...
  - `  CONDITION  ` : a [CEL condition](https://docs.cloud.google.com/resource-manager/docs/organization-policy/creating-managing-custom-constraints#common_expression_language) that is written against a representation of a supported service resource. This field can contain up to 1000 characters. For example, `"has(resource.encryptionSpec) == false"` .
...
Replace the following:
  ...
  - `  CONSTRAINT_NAME  ` : the name you defined for your custom constraint. For example, `custom.disableUnencryptedIndexes` .
...
The following example creates a custom constraint and policy that blocks creation of indexes that aren't encrypted in a given organization.
...
1.  Save the following file as `constraint-disable-unencrypted-indexes.yaml` :
    ...
    This defines a constraint that blocks the creation of Index resources that don't have an encryption specification.

2.  Apply the constraint:
    
        gcloud org-policies set-custom-constraint ~/constraint-disable-unencrypted-indexes.yaml
...
3.  Verify that the constraint exists:
    ...
    The output is similar to the following:
        ...
        custom.disableUnencryptedIndexes   DENY          CREATE         aiplatform.googleapis.com/Index   Block creation of unencrypted Indexes
        ...

### Create the policy

1.  Save the following file as `policy-disable-unencrypted-indexes.yaml` :
    
        name: organizations/ORGANIZATION_ID/policies/custom.disableUnencryptedIndexes
        spec:
          rules:
          - enforce: true
...
2.  Apply the policy:
        ...
        gcloud org-policies set-policy ~/policy-disable-unencrypted-indexes.yaml
...
3.  Verify that the policy exists:
    ...
    The output is similar to the following:
        ...
        custom.disableUnencryptedIndexes     -             SET              COCsm5QGENiXi2E=

After you apply the policy, wait for about two minutes for Google Cloud to start enforcing the policy.

### Test the policy

1.  Create an Index resource without encryption:
    
        gcloud ai indexes create \
        --metadata-file=<var>LOCAL_PATH_TO_METADATA_FILE</var> \
        --display-name=<var>INDEX_NAME</var> \
        --region=<var>LOCATION</var> \
        --project=<var>PROJECT_ID</var>
    
    The output is similar to the following:
    
        ERROR: (gcloud.ai.indexes.create) FAILED_PRECONDITION: Operation denied by org policy on resource 'projects/PROJECT_ID/locations/LOCATION': ["custom.disableUnencryptedIndexes": "This constraint blocks the creation of Index resources that aren't encrypted"]
        - '@type': type.googleapis.com/google.rpc.ErrorInfo
        domain: googleapis.com
        metadata:
            customConstraints: customConstraints/custom.disableUnencryptedIndexes
...
<tbody>
<tr class="odd">
<td>Don't allow public index endpoints.</td>
<td><pre dir="ltr" data-is-upgraded="" data-syntax="YAML" translate="no"><code>    name: organizations/ORGANIZATION_ID/customConstraints/custom.denyPublicIndexEndpoint
    resourceTypes:
    - aiplatform.googleapis.com/IndexEndpoint
    methodTypes:
    - CREATE
