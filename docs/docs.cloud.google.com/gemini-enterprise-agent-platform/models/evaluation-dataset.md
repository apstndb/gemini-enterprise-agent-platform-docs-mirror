---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/models/evaluation-dataset
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/evaluation-dataset
title: Prepare your evaluation dataset
description: Prepare evaluation datasets for the Gen AI evaluation service. Use Pandas DataFrames, Gemini, or OpenAI JSONL formats.
...
When running `client.evals.evaluate()` or `client.evals.create_evaluation_run()` , the Gen AI evaluation service automatically looks for the following common fields in your dataset:
  ...
  - `session_inputs` : (Optional) Input to initialize a [session](https://docs.cloud.google.com/agent-builder/agent-engine/sessions/overview) to run an agent. This is only optional for the `run_inference()` → `evaluate()` workflow.
