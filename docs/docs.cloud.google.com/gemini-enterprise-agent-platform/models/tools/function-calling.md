...
<!-- end list -->
  ...
  - For Gemini 3 and later models, you can include multimodal content (images and PDFs) in `functionResponse` messages that you send back to the model. For more information, see [Multimodal function responses](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/tools/function-calling#mm-fr) .

  - For Gemini 3 and later models, you can stream function call arguments as they are generated by setting `streamFunctionCallArguments` to `true` in the `functionCallingConfig` . For more information, see [Streaming function call arguments](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/tools/function-calling#streaming-fc) .
...
For Gemini 3 and later models, you can request that function call arguments be streamed as they are generated by the model, rather than waiting for the full set of arguments to be generated. This is useful for reducing perceived latency when functions need to be called.
