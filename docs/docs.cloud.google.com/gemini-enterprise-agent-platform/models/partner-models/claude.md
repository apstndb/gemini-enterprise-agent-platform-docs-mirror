...
> To see an example of using Anthropic's Claude model on Agent Platform, run the "Use Anthropic's Claude model on Agent Platform" notebook in one of the following environments:
> 
...
The Anthropic Claude models on Gemini Enterprise Agent Platform offer fully managed and serverless models as APIs. To use a Claude model on Agent Platform, send a request directly to the Agent Platform API endpoint. Because the Anthropic Claude models use a managed API, there's no need to provision or manage infrastructure.
...
The following models are available from Anthropic to use in Gemini Enterprise Agent Platform. To access a Claude model, go to its Model Garden model card.
...
Claude Sonnet 4.5 is Anthropic's Sonnet-class model for powering real-world agents, with industry leading capabilities around coding, computer use, cybersecurity, and working with office files like spreadsheets.
...
Claude Opus 4.1 is Anthropic's Opus-class model and an industry leader for coding and agent capabilities, especially agentic search. It excels for customers needing frontier intelligence:
