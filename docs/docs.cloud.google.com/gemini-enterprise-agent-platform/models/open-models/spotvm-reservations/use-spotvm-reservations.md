---
name: documents/docs.cloud.google.com/gemini-enterprise-agent-platform/models/open-models/spotvm-reservations/use-spotvm-reservations
uri: https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/open-models/spotvm-reservations/use-spotvm-reservations
title: Using Spot VMs or reservations to deploy a Gemini Enterprise Agent Platform Llama-3.1 endpoint on Cloud GPUs
description: Gemini Enterprise Agent Platform is a central console designed for platform and security administrators to build, scale, monitor, optimize, and govern the entire lifecycle of AI agents.
data_source: docs.cloud.google.com
---

This tutorial guides you through deploying the Meta-Llama-3.1-8B model on Gemini Enterprise Agent Platform. You'll learn how to deploy endpoints and optimize for your specific needs. If you have fault-tolerant workloads, you can optimize for costs by using [Spot VMs](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/training/use-spot-vms) . If you want to ensure availability, use [Compute Engine reservations](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/predictions/use-reservations) . You'll learn how to deploy endpoints that utilize:
...
For more information, see [Spot VMs](https://docs.cloud.google.com/compute/docs/instances/spot) or [Reservations of Compute Engine resources](https://docs.cloud.google.com/compute/docs/instances/reservations-overview) .
...
Before you begin, complete the following prerequisites:
  ...
  - The Gemini Enterprise Agent Platform and Compute Engine APIs enabled.
  ...
  - If you are using shared reservations, grant [IAM permissions](https://docs.cloud.google.com/compute/docs/access/iam) between projects. Those permissions are all covered in the notebook.
...
If you intend to use a reservation that was created in a *different* project within the same organization, make sure to grant the `compute.viewer` role to the P4SA (Principal Service Account) of *both* projects. The notebook code will automate this, but ensure that `SHARED_PROJECT_ID` is correctly set. This cross-project permission allows the Gemini Enterprise Agent Platform endpoint in your primary project to *see* and use the reservation capacity in the shared project.
...
![Hugging Face Access Token Settings](https://docs.cloud.google.com/static/gemini-enterprise-agent-platform/models/open-models/vllm/images/access_token_settings_link.png) **Figure 1: Hugging Face Access Token Settings**

### 3\. Deploy with Spot VM

To deploy the Llama model to a Spot VM, navigate to the "Spot VM Gemini Enterprise Agent Platform Endpoint Deployment" section in the Colab notebook and set `is_spot=True` .
...
To configure your reservations, go to the "Set Up Reservations for Gemini Enterprise Agent Platform Predictions" section of the notebook. Set the required variables for the reservation such as the `RES_ZONE` , `RESERVATION_NAME` , `RES_MACHINE_TYPE` , `RES_ACCELERATOR_TYPE` , and `RES_ACCELERATOR_COUNT` .
...
With your endpoint deployed, verify that the [reservation](https://console.cloud.google.com/compute/reservations) is used by Gemini Enterprise Agent Platform online prediction, and make sure to test a few prompts to ensure it is deployed properly.

![Check reservation is used by Vertex online prediction](https://docs.cloud.google.com/static/gemini-enterprise-agent-platform/models/open-models/spotvm-reservations/images/consumed-reservation.png) **Figure 3: Check reservation is used by Vertex online prediction**
...
## Next steps
  ...
  - Explore different [Llama 3 model variants](https://console.cloud.google.com/agent-platform/publishers/meta/model-garden/llama3-3) .
  - Learn more about reservations with this [Compute Engine Reservations Overview](https://docs.cloud.google.com/compute/docs/instances/reservations-overview) .
  - Learn more about Spot VMs with this [Spot VMs Overview](https://docs.cloud.google.com/compute/docs/instances/spot) .
